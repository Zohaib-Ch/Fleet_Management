...
export default function ActivityLogView() {
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-[var(--bg-secondary)] rounded-2xl p-6 shadow-sm border border-[var(--border-primary)]">
        <div className="mb-6">
          <h3 className="text-xl font-bold text-[var(--text-primary)]">Fleet Velocity</h3>
      ...
      <div className="bg-[var(--bg-secondary)] rounded-2xl p-8 shadow-sm border border-[var(--border-primary)]">
        <h3 className="text-xl font-bold text-[var(--text-primary)] mb-6">Activity Log</h3>
        ...
        <div className="relative pl-4 space-y-8 before:absolute before:inset-0 before:ml-8 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent">
          {activityLogs.map((log, idx) => (
            <div key={log.id} className="relative flex items-start gap-6 group">
              ...
              {/* Icon / Avatar */}
              <div className="relative z-10 w-10 h-10 shrink-0 bg-[var(--bg-secondary)] rounded-full flex items-center justify-center shadow-sm border-2 border-white ring-1 ring-slate-200">
                {log.avatar ? (
              ...
              {/* Content */}
              <div className="flex-1 bg-[var(--bg-primary)]/50 rounded-xl p-4 border border-[var(--border-primary)] hover:border-slate-300 transition-colors">
                <p className="text-sm text-secondary leading-relaxed">
                  <span className="font-bold text-[var(--text-primary)]">{log.driver}</span>{' '}
                  {log.message}
                </p>
