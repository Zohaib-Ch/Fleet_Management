...
export default function AlertsView({ alerts, setAlerts }) {
  ...
  return (
    <div className="space-y-10 view-transition pb-20">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 glass-obsidian p-10 rounded-[3rem] border-white/5 glow-purple">
        <div>
          <h2 className="text-4xl font-luxury text-white tracking-tighter italic uppercase">Risk Intelligence</h2>
          <p className="text-slate-500 font-bold uppercase tracking-[0.2em] mt-2 text-[10px]">Real-time Threat Mitigation Terminal</p>
        </div>
        ...
        <div className="flex flex-wrap gap-4 w-full lg:w-auto">
          <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10">
            {['all', 'critical', 'warning', 'info'].map(t => (
              <button 
                key={t}
                onClick={() => setFilter(t)}
                className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === t ? 'bg-primary text-white shadow-lg' : 'text-secondary hover:bg-white/5'}`}
              >
                {t}
              </button>
            ))}
          ...
          <div className="relative flex-1 lg:flex-none">
            ...
            <input 
              ...
              className="w-full lg:w-64 pl-12 pr-6 py-3.5 bg-white/[0.03] border border-white/10 rounded-2xl text-[10px] font-black text-white uppercase tracking-widest focus:bg-white/5 focus:border-purple-500/50 outline-none transition-all"
            />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 space-y-6">
          <AnimatePresence mode="popLayout">
            {filteredAlerts.map((alert) => (
              <motion.div
                ...
                className="glass-obsidian rounded-[2.5rem] p-8 border-white/5 hover:border-white/10 transition-all group relative overflow-hidden"
              >
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        <h4 className="text-xl font-luxury text-white tracking-tighter italic uppercase">{alert.title}</h4>
                  ...
                  <div className="flex flex-col md:items-end gap-4 w-full md:w-auto">
                    ...
                    <div className="flex gap-3">
                      <button 
                        ...
                        className="flex-1 md:flex-none px-6 py-3 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-white transition-all flex items-center justify-center gap-2"
                      >
                        <Check size={14} /> Resolve
                      </button>
                      <button className="p-3 bg-white/5 text-secondary border border-white/10 rounded-xl hover:text-white transition-all">
          ...
          {filteredAlerts.length === 0 && (
            <div className="py-32 flex flex-col items-center justify-center text-center glass-obsidian rounded-[3rem] border-white/5">
              ...
              <h3 className="text-3xl font-luxury text-white uppercase italic tracking-tighter">System Nominal</h3>
        ...
        <div className="space-y-8">
           <HolographicCard glowColor="purple" className="p-10 rounded-[3rem] h-[500px] flex flex-col">
              <h3 className="text-xl font-luxury text-white tracking-tighter italic uppercase mb-8">Asset Hotspots</h3>
              <div className="flex-1 bg-white/[0.02] rounded-3xl border border-white/10 relative overflow-hidden flex items-center justify-center">
              ...
              <button className="w-full py-5 bg-primary text-white border border-white/10 font-black rounded-2xl hover:bg-white/5 transition-all uppercase tracking-widest text-[10px] mt-8">
           ...
           <HolographicCard glowColor="gold" className="p-10 rounded-[3rem] bg-gradient-to-br from-amber-600/10 to-gold/10">
              <div className="flex items-center gap-4 mb-6">
                 ...
                 <h4 className="text-sm font-black text-white uppercase tracking-widest">Priority Protocols</h4>
              </div>
              <p className="text-lg font-luxury text-white mb-8">{alerts.filter(a => a.type === 'critical').length} Critical Alerts require manual commander intervention.</p>
